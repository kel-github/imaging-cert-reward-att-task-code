...
%   always be over the smallest window possible which is 128 trials. 
%   
%   tblocks = the total number iof iterations over 128 trials to perform
...
for iBlock = 1:tblocks
    ...
    probabilities  = [ repmat(1, 1, 36), repmat(2, 1, 36), repmat(3, 1, 72)];
    trial_position = [ repelem([0, 1], [28, 8]), repelem([1, 0], [28, 8]), repelem([0, 1], [36, 36]) ]; % but actually use 28 & 28 for last condition
    reward_type    =  repmat([ repelem([1, 2, 3, 4], 7), repelem([1, 2, 3, 4], 2)], 1, 4);           
    ...
    % reward_type pattern. Basically, there are 4 iterations, the first two
