function [valid, response, ts] = ...
    do_trial(wh, sess, task, cue, target, ccw, hrz, ...
...
%     EEG
    ...
    if reward > 1
        ...
        if ~any(reward)            
            cue_colour = sess.reward_colours(2,:);           
        elseif reward == 1            
            cue_colour = sess.reward_colours(1,:);
        end
    end
...
%    do_trigger(trid, labjack, triggers.cue, ts.cue);
    ...
    % Hold with just a fixation cross.
    draw_pedestals(wh, 1:2, gabor_rect, 0.5 * get_ppd(), white, grey);
    draw_stim(wh, 0, stim_dark);
    [ts.hold] = Screen('Flip', wh, ts.cue+time.cue);
